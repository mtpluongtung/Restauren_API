...
namespace Repositories.Implementations
{
    public class HoaDonRepository : IHoaDonServices
    {
		...
		public async Task<List<DoanhThuChiTiet>> DoanhThuChiTiet(DateTime request)
		{
			...
			//			join dh in _contextDonHangChiTiet.Query() on hd.MaDonHang equals dh.MaDonHang
			//			where hd.NgayTao.Date == request.Date && hd.ThanhToan
			//			group dh by new { hd.MaHoaDon, hd.TongTien } into g // Nhóm theo hóa đơn
			//			select new DoanhThuChiTiet
			//			{
			//				MaHoaDon = g.Key.MaHoaDon.ToString(),
			//				TongTien = g.Key.TongTien,
			//				ChiTietHoaDon = g.Select(dh => new ChiTietHoaDon
			//				{
			//					TenMonAn = dh.Name,
			//					Soluong = dh.SoLuong,
			//					DonGia = dh.Gia,
